...
def progress_hook(d):
    ...
    if d['status'] == 'downloading':
        ...
        status_label.config(text=f"მიმდინარეობს: {title[:30]}... {d['_percent_str']}", foreground="blue")
    elif d['status'] == 'finished':
        download_counter += 1
        status_label.config(text=f"დასრულდა: '{d.get('info_dict', {}).get('title', '')}'", foreground="green")
        count_label.config(text=f"გადმოწერილია: {download_counter} / {total_videos}", foreground="black")
...
# გადმოწერის ლოგიკები
def format_urls():
    global download_counter, total_videos
    ...
    # Reset counters and update label
    download_counter = 0
    total_videos = len(urls)
    if total_videos > 0:
        count_label.config(text=f"გადმოწერილია {download_counter} / {total_videos} ვიდეო", foreground="black")
    else:
        count_label.config(text="", foreground="black")
    ...
    if total_videos > 0:
        status_label.config(text="ყველა გადმოწერა დასრულდა!", foreground="green")
...
count_label = ttk.Label(root, text="", font=("Helvetica", 12), foreground="black")
